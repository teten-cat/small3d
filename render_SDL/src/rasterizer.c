...
void Draw_Line(int x0, int y0, int x1, int y1, uint32_t color_loaded) {
  ...
  while(1) {
    Color_Pixel(buffer, width, x0, y0, color_loaded);
...
void Fill_Polygon(vec2int p1, vec2int p2, vec2int p3, uint32_t color) {
  ...
  if(box_height == 0 && box_width == 0) {
    Color_Pixel(buffer, width, maxX, maxY, color);
    return;
  }
  if(box_height == 0) {
    Color_Band(buffer, width, minX, maxX, maxY, color);
    return;
  } else if(box_width == 0) {
  ...
  // fill the middle
  for(int i = 0; i < box_height; i++) {
    Color_Band(buffer, width, band_left[i], band_right[i], apex.y+i, color);
  }
...
void Fill_Polygon_Space(vec3 p1, vec3 p2, vec3 p3, uint32_t color) {
  ...
  if(box_height == 0) {
    Color_Band_Space(buffer, width, minX, maxX, maxY, minX_z, maxX_z, color);
    return;
  } else if(box_width == 0) {
  ...
  for(int i = 0; i < box_height; i++) {
    ...
    if (isnan(left_z) || isnan(right_z))
      continue;
    Color_Band_Space(buffer, width, band_left[i], band_right[i], apex.y+i,left_z, right_z, color);
  }
