...
void Init_Polygon_Drawer(uint32_t *buffer, float* depth_init, int width, vec2int coordinates_origin, Color *colors, int colors_num) {
  pixel_buffer = buffer;
  screen_width = width;
  ...
  for(int i = 0; i < color_array_length; i++) {
    default_colors[i] = Load_Pixel(colors[i]);
  }
}
...
void Draw_Polygon(vec2int p1, vec2int p2, vec2int p3,  Color color, DrawMode mode) {
  ...
  uint32_t loaded_color = Load_Pixel(color);
